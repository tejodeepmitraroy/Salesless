			...
			>
				...
				<div className="grid gap-6">
					{stores.map((store, i) => (
						...
						>
							<Card
								className={`cursor-pointer transition-all hover:shadow-md ${selectedStore === store.id ? 'ring-vsphere-primary ring-2' : ''}`}
								onClick={() => handleStoreSelect(store.id)}
							>
								<CardContent className="p-0">
									<div className="flex items-center p-6">
										<div className="from-vsphere-primary to-vsphere-secondary mr-4 flex h-12 w-12 items-center justify-center rounded-md bg-gradient-to-br font-bold text-white">
											{store.name.substring(0, 2).toUpperCase()}
										</div>
										<div className="flex-1">
											<h3 className="text-lg font-semibold">{store.name}</h3>
											<p className="text-muted-foreground text-sm">
												{store.role} • {store.products} products
											</p>
										</div>
						...
						>
							<CardContent className="flex items-center justify-center p-6">
								<div className="text-center">
									...
									<p className="text-muted-foreground text-sm">
										Start with a fresh store setup
									</p>
								</div>
							</CardContent>
						</Card>
